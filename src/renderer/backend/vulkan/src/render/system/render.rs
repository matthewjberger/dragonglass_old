...
pub fn render_system() -> Box<dyn Runnable> {
    SystemBuilder::new("render")
        ...
        .build_thread_local(move |_, mut world, renderer, query| {
            ...
            let camera_position = glm::vec3(10.0, 4.0, 10.0);
            ...
            for transform in query.iter(&mut world) {
                ...
                vulkan_gltf_asset.raw_asset.walk(
                    |scene_index, graph_index, node_index, node, graph| {
                        ...
                        let ubos = [ubo];
                        let buffer = &mesh_info.uniform_buffers[image_index as usize];
